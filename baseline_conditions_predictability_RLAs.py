...
class Random_base(object):
    ...
    def is_same_tree(self, randtree, abs_root):
        ...
        for edgex in randtree.edges:
            if not edgex[0] == abs_root:
                rand_tree.add_edge(edgex[0], edgex[1])  # regenerates a dummy random tree by removing its abstract root

        real_tree = nx.DiGraph()
        for edgez in self.tree.edges:
            if not edgez[0] == 0:
                real_tree.add_edge(edgez[0], edgez[1])  # regenerates a dummy real tree by removing its abstract root
    ...
    def is_similar_predictability_distribution(self, randtree, abs_root):
        ...
        for edgey in rand_tree.edges:
            random_predictability_sample.append(find.calculate_predictability(edgey))
        ...
        for edgev in real_tree.edges:
            real_predictability_sample.append(get.calculate_predictability(edgev))
        real_predictability_sample.sort()
        return random_predictability_sample == real_predictability_sample
    ...
    def rand_tree(self, num_cross_real):  # requires number of crossings from the real tree
        ...
        if self.is_equal_num_crossings(treey, abstract_root, num_cross_real):  # matches the no. of crossings in the real and random tree
            if self.is_similar_predictability_distribution(treey, abstract_root):  # check predictability distribution
    ...
    def gen_random(self, num_cross_real):  # requires numbr of crossings from the real tree
        ...
        if n < 30:
            x = 0
            while len(self.ls_rand) == 0 and x < 30000:  # checks if list of random trees is empty and limits the generating attempts 
                x = x + 1
                self.rand_tree(num_cross_real)
                rand_out = self.ls_rand
        return rand_out  # returns the list of random trees

# Ensure you have the SUD treebank data properly downloaded and organized in the specified directory
